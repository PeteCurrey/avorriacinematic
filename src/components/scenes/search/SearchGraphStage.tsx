...
export function SearchGraphStage({ progress }: SearchGraphStageProps) {
  ...
  return (
    <div
      className="absolute inset-0 w-full h-full flex items-center justify-center p-4 sm:p-10 z-20 pointer-events-auto"
      style={{ opacity }}
      aria-hidden={progress > 0.86 ? "true" : "false"}
    >
      <div className="w-full max-w-[1600px] h-[80vh] relative border border-avorria-line bg-avorria-black/90 overflow-hidden shadow-2xl p-6">
        {/* Top Marker */}
        <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-avorria-quiet border-b border-avorria-line/40 pb-3 mb-4">
        ...
        {SEARCH_NODES.map((node) => {
          ...
          return (
            <div
              ...
              className={`absolute -translate-x-1/2 -translate-y-1/2 transition-all duration-150 z-20 ${isHovered ? "scale-105 z-30" : ""}`}
          ...
          <div className="pointer-events-none">
            {SEARCH_QUERIES.map((q) => (
              <div
                ...
                style={{ left: `${q.fromX}%`, top: `${q.fromY}%` }}
              >
                QUERY: &quot;{q.query}&quot; →
              </div>
            ))}
