...
export function FeaturedProjectSplit({ project }: FeaturedProjectProps) {
  return (
    <article className="group relative border-b border-avorria-line py-16 sm:py-28">
      <div className="max-w-[1760px] mx-auto px-6 sm:px-12 lg:px-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
          {/* Left Column: Narrative & Spatial Metadata */}
          <div className="lg:col-span-6 order-2 lg:order-1 space-y-8">
            ...
            <div className="space-y-4 border-t border-avorria-line/40 pt-6">
              <p className="font-body text-base text-avorria-white/90 leading-relaxed max-w-lg">
                {project.shortSummary}
              </p>
              <p className="font-mono text-xs text-avorria-muted uppercase tracking-wider leading-relaxed max-w-lg">
                {project.role}
              </p>
            </div>

            {/* Spatial Data Metrics */}
            <div className="grid grid-cols-2 gap-4 border-t border-avorria-line/40 pt-6 font-mono text-[11px]">
              <div>
                <span className="text-avorria-quiet uppercase tracking-wider block">DISCOVERY TYPE</span>
                <span className="text-avorria-white font-bold">SPATIAL DATA ENGINE</span>
              </div>
              <div>
                <span className="text-avorria-quiet uppercase tracking-wider block">VALUATION MODEL</span>
                <span className="text-avorria-signal font-bold">AUTOMATED AVM</span>
              </div>
            </div>
            ...
            <div className="pt-2">
              ...
              >
                <span>EXPLORE SPATIAL CASE STUDY</span>
                <span className="text-avorria-signal transition-transform duration-200 group-hover:translate-x-1">→</span>
              </Link>
            </div>
          </div>

          {/* Right Column: High-Res Spatial Image Container */}
          <div className="lg:col-span-6 order-1 lg:order-2">
            <Link
              href={`/work/${project.slug}`}
              className="block relative w-full aspect-[4/3] overflow-hidden bg-avorria-surface border border-avorria-line/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-avorria-signal"
              aria-label={`View $<BreakableTitle text={project.title} /> case study`}
            >
              <Image
                src={project.heroMedia}
                alt={`$<BreakableTitle text={project.title} /> - Property Intelligence`}
                fill
